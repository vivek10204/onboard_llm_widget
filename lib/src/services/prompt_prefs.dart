...
class PromptPrefs {
  ...
  //TBDO static const _kOutputSchema = 'prompt_output_schema';

  // SAVE
  static Future<void> save({
    String? preamble,
    String? inputCsv,
    //TBDO String? outputSchema,
  }) async {
    ...
    //TBDO if (outputSchema != null) await prefs.setString(_kOutputSchema, outputSchema);
  }
  ...
  */
  ...
  // LOAD (all at once)
  static Future<({String? preamble, String? inputCsv})> loadAll() async {
    final prefs = await SharedPreferences.getInstance();
    return (
    preamble: prefs.getString(_kPreamble),
    inputCsv: prefs.getString(_kInputCsv),
    //TBDO outputSchema: prefs.getString(_kOutputSchema),
    );
  }
}
